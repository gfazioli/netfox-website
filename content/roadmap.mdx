...
Netfox **0.5** is live. The current build covers:
...
- **Menu bar** — a quick-glance popover with your network's status (devices online, risk, public IP, alerts) plus a live chart of your Mac's traffic — without opening the main window. Two icon styles: an adaptive monochrome glyph or the colour fox.

## What's next

### v0.6 — Wi-Fi diagnostics

The Wi-Fi tab today is a basic neighbour list. This release turns it into a real diagnostics tool: signal-strength history, channel usage, congestion warnings, hidden network detection. Goal: answer "is my Wi-Fi the problem?" without leaving Netfox.

### v0.7 — Link diagnostics
...
The plan past v0.8 is intentionally less specific. Likely directions:

- **Threat intelligence** — cross-referencing devices and traffic against public reputation feeds, with the helper from v0.8 doing the heavy lifting.

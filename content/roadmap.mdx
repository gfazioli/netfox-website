...
- **Discovery & history** — every device you've ever had on the network is remembered with a timeline, discovered across multiple protocols. The timeline now records **port-state changes** too, so you can see when a service appeared or went away. Discovery also **asks** now, not only listens: sweeping a range of addresses sends one echo request to each, so a device that is powered on, silent and has never talked to your Mac stops being invisible by construction — and the range doubles as a scope, so setting it to two addresses shows you those two devices and nothing else.
